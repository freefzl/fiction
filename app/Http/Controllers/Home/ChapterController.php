...
//        dump($model);
    ...
    public function xq($id)
    {
        ...
        $chapter = NovelChapter::where(['is_up'=>1])->where(['bid'=>$id])->where(['goId'=>'0'])->where(['is_pay'=>0])->orderBy('id','desc')->limit(9)->get();


        $new_chapter = NovelChapter::where(['is_up'=>1])->where(['bid'=>$id])->orderBy('id','desc')->limit(15)->get();
    ...
    public function chapter_list($id)
    {
        ...
        $chapter = NovelChapter::where(['is_up'=>1])->where(['bid'=>$id])->get();
