    ...
    public static function updateBooks()
    {
        ...
        foreach ($types as $type){
            ...
            if(!$is_type){
                ...
                $id = NovelBook::where(['type_id'=>$type->id])->where(['is_up'=>1])->orderBy('updated_at','desc')->limit(1)->first();
        ...
        $id = NovelBook::where(['is_up'=>1])->orderBy('updated_at','desc')->limit(1)->first();
        if($id->id){
            NovelChapter::where(['bid'=>$id->id])->update(['is_up'=>1]);
        }
